package kuhcorp.template.domain.user;

import jakarta.persistence.Entity;
import jakarta.persistence.Enumerated;
import jakarta.persistence.Table;
import jakarta.validation.constraints.NotNull;
import kuhcorp.template.auth.provider.custom.AuthDtos.RegisterReq;
import kuhcorp.template.db.EntityWithMetadata;
import kuhcorp.template.db.EntityWithStatus;
import lombok.Getter;

import static jakarta.persistence.EnumType.STRING;

@Entity
@Table(name = "app_user")
public class User extends EntityWithMetadata implements EntityWithStatus {

    @NotNull
    @Getter
    private String email;

    @NotNull
    @Getter
    private String passwordHash;

    @Getter
    private String firstName;

    @Getter
    private String lastName;

    @NotNull
    @Getter
    private Boolean isAdmin;

    @NotNull
    @Enumerated(STRING)
    private UserStatus status;

    public static User create(RegisterReq req, String passwordHash) {
        return create(req, passwordHash, false);
    }

    public static User create(RegisterReq req, String passwordHash, boolean admin) {
        var u = new User();
        u.email = req.getEmail();
        u.passwordHash = passwordHash;
        u.firstName = req.getFirstName();
        u.lastName = req.getLastName();
        u.isAdmin = admin;
        u.status = UserStatus.ACTIVE;
        return u;
    }

    public String getUsername() {
        return email;
    }

    @Override
    public boolean isActive() {
        return status.isActive();
    }
}
